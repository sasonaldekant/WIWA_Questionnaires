/*
    WIWA – Questionnaire JSON generator (v7) – Model 18-01-2026
    ---------------------------------------------------------------------------
    Purpose
...
DECLARE @QuestionnaireTypeID SMALLINT = 1; -- << SET THIS
...
DECLARE @RulesJson NVARCHAR(MAX) = (
    SELECT
        ...
        -- Matrix Data if inline? No, matrix is in referenceTables.
...
SELECT
    ...
    (
        SELECT
            meta = JSON_QUERY((
                SELECT
                    schemaVersion        = N'v3_MODEL_18_01_2026',
